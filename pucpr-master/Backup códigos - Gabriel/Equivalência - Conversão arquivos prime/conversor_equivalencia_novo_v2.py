    ...
    try:
        for p in range(len(valores)):
            if p < 20:
                ...
                valores[p].append(dados[p-19])
            else:
                valores[p].append(res_equivalencia[p-24])
        return valores
    except:
        return valores
...
def main():
    ...
    planilha = askopenfilename(
        filetypes=[('Arquivo excel', '*')],
        title='Selecione a planilha de alunos')

    curso = str(planilha).split('/')[-2]
    os.makedirs(f'{curso}_v2', exist_ok=True)
    nome_arquivo = str(planilha).split('/')[-1]
    # Mostra qual foi o arquivo selecionado
    print(f"Arquivo selecionado: {nome_arquivo}")
    turma = f"{nome_arquivo.split('_')[-2]} {nome_arquivo.split('_')[-1].split('.')[0]}"
    ...
    df = pd.read_excel(planilha, dtype=str, sheet_name='Ciclos', skiprows=5, usecols=lambda x: 'Unnamed' not in x)
    df.dropna(inplace=True, how='all')
    df = df.fillna("-")
    '''n_df = df.to_excel('meh.xlsx', index=False)
    exit()'''
    ...
    n_df.to_excel(f'{curso}_v2/equivalencias_{turma}.xlsx', index=False)
    print('Excel gerado')
...
#TODO: Add o Ciclo
#TODO: Arrumar para pegar os valores direito
#TODO: Alterar a geração do nome do arquivo