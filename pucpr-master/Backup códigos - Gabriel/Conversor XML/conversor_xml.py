...
def remove_dados_zerados(root):
    ...
    for lesson in root.iter("lesson"):
        ...
        # pegar as diciplinas que não tem sala alocada e mudar para aula remota
        extra = ['AULA ESPECIALIZAÇÃO','PERMANÊNCIA']

        if all_subjects.get(lesson.get('subjectid')) not in extra:
