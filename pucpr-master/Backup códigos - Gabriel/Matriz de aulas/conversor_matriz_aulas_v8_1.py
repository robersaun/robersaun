...
'''Esse possui o inicio de uma caixa de dialogo para inserção deos dados de escola e curso'''
...
# TODO: Verificar se todas as funções estão sendo inseridas
# Insere as funções nas células
def ajusta_funcoes(df_para):
    inicio = 14
    fim = 6
    for col_para in df_para.head():
        # Faltam algumas colunas para colocar
        # Essas inserções de função não estão fucionando >> Provavelmente pq a tabela 2 está sendo identificada dps 
        # e está sobreescrevendo os dados, então a inserção da função funciona, só não aparece
...
def nome_curso():
    i = simpledialog.askstring('Insira a string', 'Escreva o nome do curso')
    print(i)


def insere_template(load_df_para, load_template):
    fd_para_pagina_origem = load_df_para["Sheet1"]
    fd_para_pagina_destino = load_template['CRIAÇÃO DE MATRIZ PRESENCIAL']

    root = Tk()

    botao = Button(root, text='Nome do Curso', command=nome_curso)
    botao.pack()
...
def main():
    ...
    load_df_para = load_workbook('template_matriz.xlsx')
