...
def recupera_colunas(df):
    ...
    novas_colunas = ['Tipo de equivalência', 'Grade da equivalência', 'Ciclo da equivalência',
                     ...
                     'CH Relógio Oficial da equivalência', 'Porcentagem CH R', 'Ponto de Atenção', 'Observação']
...
def calc_porcentagem(chr_e, chr_b):
    resultado = int((chr_b / chr_e) * 100)
    return resultado
...
def compara_equivalencia(df, linha, index, tipo):
    ...
    porcentagem = calc_porcentagem(ch_ro, ch_ro_r)
    ...
    if 'N' in tipo or 'n' in tipo:
        ...
        return [f'{cr_r} ({cr_eq})', f'{ch_t_r} ({ch_t_eq})', f'{ch_p_r} ({ch_p_eq})',
                f'{ch_o_r} ({ch_o_eq})', f'{ch_ro_r} ({ch_ro_eq})', f'{porcentagem}%']
...
def resultado_equivalencia(df, linha, dados):
    try:
        ...
        v_porcentagem = int(res_equivalencia[-1].split('%')[0])

        if (ano_matriz >= 2020 and v_porcentagem < 50) or (ano_matriz < 2020 and v_porcentagem < 60):
            res_equivalencia.append('Sim')
        else:
            res_equivalencia.append('Não')
...
def main():
    ...
    n_df.to_excel(f'Equivalencias-{planilha.split("-")[-2].split(".")[0]}-pCHR-v3.xlsx', index=False)
    print('Excel gerado')
