...
def recupera_colunas(df):
    ...
    novas_colunas = ['Tipo de equivalência', 'Grade da equivalência', 'Ciclo da equivalência',
                     ...
                     'CH Relógio Oficial da equivalência', 'Porcentagem Crédito', 'Ponto de Atenção']
...
def compara_equivalencia(df, linha, index, tipo):
    ...
    porcentagem = calc_porcentagem(cr, cr_r)
...
def main():
    ...
    n_df.to_excel(f'Equivalencias-{planilha.split("-")[-1].split(".")[0]}-pCredito.xlsx', index=False)
    print('Excel gerado')
