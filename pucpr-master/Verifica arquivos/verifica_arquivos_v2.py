...
def lista_matrizes(root):
    ...
    # Identifica o ano e semestre atual
    data_atual = datetime.today()
    ano_atual = str(data_atual.year)
    mes_atual = int(data_atual.month)
    ...
    periodo_atual = f'{ano_atual}.{semestre_atual}'

    lista_profs, lista_pendencias = [], []
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        # Verifica quais anos não estão presentes na lista de documentos existentes
        while ano_verifica <= int(ano_atual):
            # Para os casos em que o semestre da admissão for o segundo
            ...
            # Para os casos em que o semestre atual for o primeiro
            if str(ano_verifica) + '.1' == periodo_atual:
                break
            else:
                if str(ano_verifica) + '.1' not in str(ano_periodo):
                    ano_periodo_faltante.append(f'{ano_verifica}.1')
                if (str(ano_verifica) + '.2' not in str(ano_periodo) and
                        str(ano_verifica) + '.2' != periodo_atual):
                    ano_periodo_faltante.append(f'{ano_verifica}.2')
            ano_verifica += 1
    ...
    # Verifica quais professores listados no documento tem diretório de arquivos
    for nome_prof in prof_relatorio:
        if nome_prof not in lista_profs:
            lista_profs.append(nome_prof)
