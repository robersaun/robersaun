'''
Script recebe um diretório dos professores e também recebe um excel com os professores ativos e a data de admissão. 
Com isso o script verifica cada arquivo presente na pasta de cada professor e retorna se determinado professor apresenta documentos faltantes
...
def lista_matrizes(root):
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        # Verifica o nome da pasta e corrrige a fonte para maiúscula
        ...
        if '-' in ano_mes_admissao:
            ...
            if ano_mes_admissao == '2015-1':
                admissao.append('Não foi encontrado')
            else:
                admissao.append(f'{ano_mes_admissao.split("-")[2]}/{mes_admissao}/{ano_admissao}')
        else:
            mes_admissao = int(ano_mes_admissao.split('/')[1])
            ano_admissao = int(ano_mes_admissao.split('/')[2])
            admissao.append(ano_mes_admissao)
