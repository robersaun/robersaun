    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        for file in filenames:
            if file.startswith("Grade_curricular_") and file.endswith(".xlsx"):
                possui_matriz = True
                caminho_matriz = file
            elif "Res" in file and "CONSUN" in file:
        ...
        if possui_matriz and possui_resolucao and not possui_relatorio_erros:
            ...
            try:
                compara_matrizes(root, dirpath, dirpath + "/" + caminho_matriz, dirpath + "/" + caminho_resolucao)
                contagem += 1
...
def compara_matrizes(root, dirpath, arquivo_excel, arquivo_word):
    ...
    nome_arquivo_saida = f"{dirpath}/Relatório de Erros.xlsx"

    workbook_excel = op.load_workbook(arquivo_excel)
    sheet_excel = workbook_excel["Ciclos"]

    workbook_word = op.load_workbook(arquivo_word)
    sheet_word = workbook_word["Plan1"]
    ...
    for linha in range(1, sheet_word.max_row):
        if type(sheet_word.cell(linha, 1).value) is str:
            if "PERÍODO" in str.upper(sheet_word.cell(linha, 1).value):
                ciclo = int(
                    sheet_word.cell(linha, 1).value.replace("°", "|").replace("º", "|").replace(".", "").split("|")[0])
            continue
        if str(sheet_word.cell(linha, 1).value) == "None" or \
                str(sheet_word.cell(linha, 2).value) == "None" or \
                str(sheet_word.cell(linha, 1).value) == "Ordem" or \
                str(sheet_word.cell(linha, 1).value) == "TOTAL" or \
                str(sheet_word.cell(linha, 1).value) == "":
            continue
        ordem_disciplina = int(sheet_word.cell(linha, 1).value)
        nome_disciplina = str(sheet_word.cell(linha, 2).value).strip().replace("–", "-")
        requisito_disciplina = str(sheet_word.cell(linha, 3).value) \
            .replace(" ", "").replace("\xa0", "").replace(";", ",").strip("0").replace("None", "")
        aula_teorica = int(sheet_word.cell(linha, 4).value)
        aula_pratica = int(sheet_word.cell(linha, 5).value)
        creditos = int(sheet_word.cell(linha, 6).value)
        ch_oficial = int(sheet_word.cell(linha, 7).value)
        ch_relogio_oficial = int(sheet_word.cell(linha, 8).value)
    ...
    for linha in range(1, sheet_excel.max_row):
        if str(sheet_excel.cell(linha, 2).value) == "" or \
        ...
        nome_disciplina = str(sheet_excel.cell(linha, 2).value).strip().split(" - ")[1:]
        separador_nome = " – "
        nome_disciplina = separador_nome.join(nome_disciplina).strip().replace("–", "-")

        req = str(sheet_excel.cell(linha, 20).value).replace('None', '')
        requisito_disciplina = tratar_requisitos(req, disciplinas_word)

        aula_teorica = sheet_excel.cell(linha, 7).value
        aula_pratica = sheet_excel.cell(linha, 8).value
        creditos = sheet_excel.cell(linha, 18).value
        ch_oficial = sheet_excel.cell(linha, 9).value
        ch_relogio_oficial = sheet_excel.cell(linha, 13).value

        if str(aula_teorica) == "None" or str(aula_teorica) == "":
            aula_teorica = 0
        ...
        if str(aula_pratica) == "None" or str(aula_pratica) == "":
            aula_pratica = 0
        ...
        if str(creditos) == "None" or str(creditos) == "":
            creditos = 0
        else:
            creditos = int(creditos)

        if str(ch_oficial) == "None" or str(ch_oficial) == "":
            ch_oficial = 0
        else:
            ch_oficial = int(ch_oficial)

        if str(ch_relogio_oficial) == "None" or str(ch_relogio_oficial) == "":
