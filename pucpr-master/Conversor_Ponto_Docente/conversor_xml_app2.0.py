...
ARQUIVO PONTO.xlsx e base professor - escola.xlsx
...
def remove_dados_zerados(root,escola):
    ...
    #dict para armazenar o id da subject para trazer o name dela
    for subject in root.iter("subject"):
    ...
    for cards in root.find("cards").findall("card"):
        if cards.get('days') == '0000001':
            card_idlesson_flutuante.append((cards.get('lessonid')))
            f.write(f"Aviso[ação]: o card '{cards.get('lessonid')},"
                    f"é GRADUAÇÃO e tem HORARIO FLUTUANTE então vai ser excluido\n")
            root.find("cards").remove(cards)

    win['progbar'].update_bar(45)

    aula_esp = ['9999;99;AULA ESPECIALIZAÇÃO*1*;AULESP1', '9999;99;AULA ESPECIALIZAÇÃO*2*;AULESP2',
                '9999;99;AULA ESPECIALIZAÇÃO*3*;AULESP3']


    #armazenando o id da subjetc e lesson que são flutuantes e não são aula externa
    for lesson in root.iter("lesson"):
    ...
    #excluir lessons que são horario flutuante
    for lesson in root.find("lessons").findall("lesson"):
        if lesson.get('id') in lesson_flu_graduação:
            f.write(f"Aviso[ação]: a lesson '{lesson.get('id')},"
    ...
    # mudando o id das disciplinas que não tem sala alocada para AULA EXTERNA
    for lesson in root.iter("lesson"):
        if lesson.get('id') in lista_aulaexterna and lesson.get('id') not in card_idlesson_flutuante:
            f.write(f"Aviso[ação]: a lesson ID='{lesson.get('id')}, subject ID={all_subjects[lesson.get('subjectid')]}"
    ...
    for cards in root.find("cards").findall("card"):
        if cards.get('lessonid') in lista_aulaexterna and cards.get('classroomids') != "":
            f.write(
    ...
    #Identifica se o name da subject esta no formato errado, se tiver, exclui e avisa no log
    for subject in root.iter("subject"):
        all_subjects[subject.get("id")] = subject.get("name").split(';')
        try:
...
def cria_dicionario_professor_escola():
    ...
    dataframe = pandas.read_excel("Base_de_dados/base professor - escola.xlsx", engine="openpyxl")
...
def cria_dicionario_escola(escola):
    # Lê o arquivo excel que contêm os dados
    dataframe = pandas.read_excel("Base_de_dados/ARQUIVO PONTO.xlsx", engine="openpyxl",sheet_name="CURSOS")
...
def main(arquivo_xml,escola):
    ...
    xml_convertido.write(f"XML_gerados/{nome_arquivo}_Arquivo_gerado.xml", encoding="windows-1252")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if event is None:
            ...
            f = open(f"Log_gerados/{nome_arquivo}_Log.txt", "w+", encoding='utf8')
