...
print("Selecione o relatório de disciplinas")

relatorio_disciplinas = askopenfilename(
    filetypes=[('Arquivo excel', '.xlsx')],
    title='Selecione o relatório de disciplinas')
...
df_joined = pd.merge(left=df_alunos, right=df_disciplinas, left_on='Matrícula', right_on='CODIGO')

# modificando o dataframe
df_joined = df_joined[['Estabelecimento', 'Escola', 'Centro de Resultado', 'Curso', 'Série', 'Matrícula',
                       'Nome Completo', 'CPF', 'Data de Nascimento', 'E-mail', 'Telefone Celular', 'Situação Acadêmica',
...
df_joined['CR_Disciplina'] = df_joined.apply(lambda row: encontra_cr(row["Curso_Disciplina"], row["TURMA_DISCIPLINA"]),
                                             axis=1)
